...
where the determinstic model for $\mu_{(t)}$ is a Gompertz model of population growth that predicts the mean of $z_{(t)}$ on the log scale as function of the true state of the log population at time $t-1$ ($z_{(t-1)}$), the intrinsic, per capita rate of increase (*r*), the strength of density-dependence ($b_0$), the effect ($b_1$) of mean annual snow water equivalent at time *t* ($x_{(t)}$), and $e_{(t)}$ is the logarithmic integration of extractions occuring between observations in year $t-1$ and year $t$.
